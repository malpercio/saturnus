...
class TimeHandler{
  ...
  partialPrettify(typeHandler){
    let readable = "",
      firstDone = false;
    for(let range of this.ranges){
      if(range.isAll(this.allowed)){
        return "every " + typeHandler;
      }
      else if(range.isPunctual()){
        if(!firstDone){
      ...
      else if(range.isOneStepRange()){
        readable += firstDone? ", " : "";
        readable += "every " + typeHandler + " from " + range.start + " to " + range.end;
      }
      else if(range.isRangeCycle()){
        readable += firstDone? ", " : "";
        readable += "every " + range.step + " " + typeHandler + "s from " + range.start + " to " + range.end;
      }
      firstDone = true;
    }
    return readable;
